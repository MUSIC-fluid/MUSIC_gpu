    ...
    for (int a = 1; a < 4; a++) {
        for (int b = a; b < 4; b++) {
            sigma_local[a][b] = ((dUsup_local[a][b] + dUsup_local[b][a])/2.
                - (DATA_ptr->gmunu[a][b] + u_local[a]*u_local[b])
                  *theta_u_local/3.
                + u_local[0]/tau*DATA_ptr->gmunu[a][3]*DATA_ptr->gmunu[b][3]
                - u_local[3]/tau/2.
                  *(DATA_ptr->gmunu[a][3]*DATA_ptr->gmunu[b][0] 
                    + DATA_ptr->gmunu[b][3]*DATA_ptr->gmunu[a][0])
                + u_local[3]*u_local[0]/tau/2.
                  *(DATA_ptr->gmunu[a][3]*u_local[b] 
                    + DATA_ptr->gmunu[b][3]*u_local[a])
                - u_local[3]*u_local[3]/tau/2.
                  *(DATA_ptr->gmunu[a][0]*u_local[b] 
                    + DATA_ptr->gmunu[b][0]*u_local[a])
